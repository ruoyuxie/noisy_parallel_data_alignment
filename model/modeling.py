...
# Modifications copyright (C) 2023 Ruoyu Xie
#
...
from model.activations import gelu, gelu_new, swish
from model.configuration_bert import BertConfig
from model.file_utils import add_start_docstrings, add_start_docstrings_to_callable
from model.modeling_utils import PreTrainedModel
from model.sparsemax import sparsemax, entmax15
...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        # Take the dot product between "query" and "key" to get the raw-noisy attention scores.
        ...
        # seem a bit unusual, but is taken from the raw-noisy Transformer paper.
...
@add_start_docstrings(
    "The bare Bert Model transformer outputting raw-noisy hidden-states without any specific head on top.",
...
class BertGuideHead(nn.Module):
    ...
    def forward(
        ...
        def get_average(attention_matrix, bias_matrix):
            actual_matrix = (bias_matrix > 0).to(torch.float32) * attention_matrix
            matrix_average = actual_matrix[actual_matrix != 0].mean()
            return matrix_average

        def create_bias_matrix(len_src, len_tgt):
            if len(len_src) != len(len_tgt):
                print("ERROR: src and tgt have different length")
                return

            max_src_len = int(max(len_src).item())
            max_tgt_len = int(max(len_tgt).item())
            bias_matrix_set = torch.zeros(len(len_src), 1, max_src_len + 2, max_tgt_len + 2)
            index = 0

            for src, tgt in zip(len_src, len_tgt):
                len_src_true = int(src.item())
                len_tgt_true = int(tgt.item())
                bias_matrix = torch.zeros(len_src_true, len_tgt_true)
                for row in range(len_src_true):
                    for column in range(len_tgt_true):
                        bias_matrix[row][column] = math.exp(h(row + 1, column + 1, len_tgt_true, len_src_true))

                # sum of the row
                z = torch.sum(bias_matrix, dim=0)
                # normalization for each column
                bias_matrix = bias_matrix / z
                # padding
                bias_matrix_set[index][0] = torch.nn.functional.pad(input=bias_matrix,
                                                                    pad=(1, max_tgt_len + 1 - len_tgt_true, 1,
                                                                         max_src_len + 1 - len_src_true),
                                                                    mode='constant', value=0)
                index += 1

            return bias_matrix_set
        ...
        bias_matrix = create_bias_matrix(len_src, len_tgt)

        # att
        attention_scores = torch.matmul(query_src, key_tgt.transpose(-1, -2))

        attention_scores += attention_scores * bias * bias_matrix * get_average(attention_scores, bias_matrix)
        ...
        attention_probs_src = nn.Softmax(dim=-1)(attention_scores_src) if extraction == 'softmax' else entmax15(
            attention_scores_src, dim=-1)
        attention_probs_tgt = nn.Softmax(dim=-2)(attention_scores_tgt) if extraction == 'softmax' else entmax15(
            attention_scores_tgt, dim=-2)
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def forward(
        ...
        if inputs_tgt is None:
            ...
            masked_lm_loss = torch.sum(masked_lm_loss.view(batch_size, -1), -1) / (
                    torch.sum(labels_src.view(batch_size, -1) != -100, -1).float() + 1e-9)  # ignore_index=-100
        ...
        sco_loss = self.guide_layer(outputs_src, outputs_tgt, inputs_src, inputs_tgt, guide=guide,
                                    extraction=extraction, softmax_threshold=softmax_threshold, bias=bias,train_so=train_so,
                                    train_co=train_co)
        return sco_loss

    def get_aligned_word(self, inputs_src, inputs_tgt, bpe2word_map_src, bpe2word_map_tgt, device, src_len, tgt_len,
                         align_layer=8, bias=0, extraction='softmax', softmax_threshold=0.001, test=False, output_prob=False,
                         word_aligns=None):
        batch_size = inputs_src.size(0)
