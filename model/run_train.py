...
from model.train_utils import _sorted_checkpoints, _rotate_checkpoints, WEIGHTS_NAME, AdamW, get_linear_schedule_with_warmup
from model.configuration_bert import BertConfig
from model.modeling import BertForMaskedLM
...
class LineByLineTextDataset(Dataset):
    def __init__(self, tokenizer: PreTrainedTokenizer, args, file_path, gold_path):
        ...
        if args.cache_data and os.path.isfile(cache_fn) and not args.overwrite_cache:
            logger.info("Loading cached alignment_data from %s", cache_fn)
            self.examples = torch.load(cache_fn)
        else:
            # Loading text alignment_data
            self.examples = []
            with open(file_path, encoding="utf-8") as f:
                lines = f.readlines()
            
            # Loading gold alignment_data
            ...
            for line_id, line in tqdm(enumerate(lines), desc='Loading alignment_data', total=len(lines)):
            ...
            if args.cache_data:
                logger.info("Saving cached alignment_data to %s", cache_fn)
...
def mask_tokens(inputs: torch.Tensor, tokenizer: PreTrainedTokenizer, args, langid_mask=None, lang_id=None) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    Prepare masked tokens inputs/labels for masked language modeling: 80% MASK, 10% random, 10% raw-noisy.
    """
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    logger.info(
        "  Total train batch size (w. parallel-noisy, distributed & accumulation) = %d",
    ...
    model_to_resize = model.module if hasattr(model, "module") else model  # Take care of distributed/parallel-noisy training
    ...
    def backward_loss(loss, tot_loss):
        if args.n_gpu > 1:
            loss = loss.mean()  # mean() to average on multi-gpu parallel-noisy training
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    )  # Take care of distributed/parallel-noisy training
...
def main():
    ...
    # Required parameters
    parser.add_argument(
        "--train_data_file", default=None, type=str, required=True, help="The input training alignment_data file (a text file)."
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    # Supervised settings
    parser.add_argument(
        "--train_gold_file", default=None, type=str, help="Gold alignment for training alignment_data"
    )
    parser.add_argument(
        "--eval_gold_file", default=None, type=str, help="Gold alignment for evaluation alignment_data"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--eval_data_file", default=None, type=str, help="An optional input evaluation alignment_data file to evaluate the perplexity on (a text file).",
    )
    parser.add_argument(
    ...
    if args.eval_data_file is None and args.do_eval:
        raise ValueError(
            "Cannot do evaluation without an evaluation alignment_data file. Either supply a file to --eval_data_file "
    ...
    if args.tokenizer_name:
        ...
        raise ValueError(
            "You are instantiating a new {} tokenizer. This is not supported, but you can do it from another data_processing, save it,"
