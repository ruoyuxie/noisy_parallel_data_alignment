...
def word_align(args, model: PreTrainedModel, tokenizer: PreTrainedTokenizer):
    ...
    for batch in dataloader:
        with torch.no_grad():
            ...
            word_aligns_list = model.get_aligned_word(ids_src, ids_tgt, bpe2word_map_src, bpe2word_map_tgt, args.device, 0, 0, align_layer=args.align_layer, extraction=args.extraction, softmax_threshold=args.softmax_threshold, bias=args.bias, test=True, output_prob=(args.output_prob_file is not None))
...
def main():
    ...
    # Required parameters
    parser.add_argument(
        "--data_file", default=None, type=str, required=True, help="The input alignment_data file (a text file)."
    )
    parser.add_argument(
        "--output_file",
        type=str,
    ...
    parser.add_argument("--bias", type=float, default=0.0, help="diagonal bias for alignment extraction")
    ...
    parser.add_argument("--num_workers", type=int, default=4, help="Number of workers for alignment_data loading")
